...
public class HelpOpCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(cmd.getName().equalsIgnoreCase(helpop)){
			...
			if(args.length < 1){
				p.sendMessage(plugin.prefix + ChatColor.RED + "Correct usage for helpop is '/helpop <what you need help with>'!");
				return true;
			}
			...
			if (helpopCD.containsKey(p.getUniqueId())) {
				...
				cdleftm = cdleftm + 1;
				if (cdleftm == 2) {
					p.sendMessage(plugin.prefix + ChatColor.RED + "You are currently on cooldown for " + cdleftm + " minutes.");
				}
				else {
					p.sendMessage(plugin.prefix + ChatColor.RED + "You are currently on cooldown for " + cdlefts + " seconds.");
				}
				return true;
			}
			if(GameUtils.alerts.size() == 0){
				p.sendMessage(plugin.prefix + "Helpop could not be sent as their is no one with alerts toggled on.");
			}else{
				p.sendMessage(plugin.prefix + "Helpop Sent!");
					...
					public void run() {
						if (helpopCD.containsKey(p.getUniqueId())) {
							if (helpopCD.get(p.getUniqueId()) <= 0) {
								p.sendMessage(plugin.prefix + ChatColor.GREEN + "You are no longer on cooldown.");
			...
			for(Player all : Bukkit.getOnlinePlayers()){
				if(!GameUtils.alerts.contains(all))return true;
